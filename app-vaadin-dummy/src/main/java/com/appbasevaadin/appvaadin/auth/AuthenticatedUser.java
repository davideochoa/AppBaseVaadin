...
/**
 * Holds the (dummy) token pair server-side in {@link VaadinSession} (never browser storage),
 * mirroring the real app-vaadin's session-based auth. Claims are decoded locally for UI gating
 * only — there is no backend here to validate a signature against.
 */
@Component
